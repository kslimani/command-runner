...
var waitDonePlugin = require('./plugins/wait/done')
...
var CommandRunner = function(name, options) {
  ...
  this._task = this._buildAsyncTask()
}
...
CommandRunner.prototype._addDefaultPlugins = function() {
  ...
  this.addPlugin('wait', 'done', waitDonePlugin)
}
...
CommandRunner.prototype._buildAsyncTask = function() {
  // Async library compliant task function which receives one or two arguments
  // For more details, see https://caolan.github.io/async/v3/docs.html#auto
  var task = (cbOrResults, cb) => {
    // Assume callback if single argument
    if (cb === undefined) {
      cb = cbOrResults
    }
